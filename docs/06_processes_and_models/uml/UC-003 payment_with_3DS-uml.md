...
sequenceDiagram
    ...
    participant Sync as Status Sync Job
    ...
    alt Existing COMPLETED idempotency record
        ...
        alt Lock rejected
            ...
            PS->>Rules: Evaluate 3DS policy<br/>merchant + risk + SCA + payment method
            Rules-->>PS: 3DS decision

            alt Missing required authentication context
                PS->>IDS: Store validation error response
                PS-->>API: 422 MISSING_AUTHENTICATION_CONTEXT
                API-->>Merchant: 422 MISSING_AUTHENTICATION_CONTEXT
            else Payment terminal
                ...
                alt Active Attempt exists
                    ...
                    PS->>RS: Request 3DS-capable route(payment, merchant_config, 3ds_decision)
                    RS->>Rules: Evaluate routing/business policies
                    Rules-->>RS: Eligible PSP candidates
                    RS->>RS: Filter by 3DS capability<br/>currency + amount + country + capture mode

                    alt No 3DS-capable route
                        RS-->>PS: NO_3DS_CAPABLE_ROUTE_AVAILABLE
                        PS->>Outbox: Store payment.processing_failed
                        PS->>IDS: Store failure response
                        PS-->>API: 422 NO_3DS_CAPABLE_ROUTE_AVAILABLE
                        API-->>Merchant: 422 NO_3DS_CAPABLE_ROUTE_AVAILABLE
                    else Route selected
                        ...
                        PS->>PA: Execute PaymentAttempt<br/>with 3DS params + psp_idempotency_key
                        PA->>PSP: Authorize / Charge + 3DS authentication data

                        alt Frictionless approved
                            PSP-->>PA: Approved + authentication result
                            PA-->>PS: Normalized SUCCESS

                            PS->>DB: Begin transaction
                            PS->>TS: Create Transaction SUCCESS<br/>authentication_result=FRICTIONLESS
                            ...
                            PSP-->>PA: REQUIRES_ACTION + next_action + psp_reference
                            PA-->>PS: Normalized REQUIRES_ACTION

                            PS->>DB: Begin transaction
                            PS->>TS: Create Transaction PENDING<br/>type=AUTHENTICATION
                            TS->>DB: Insert append-only Transaction
                            PS->>DB: Store Authentication Session ACTIVE
                            ...
                            PS->>Outbox: Store transaction.created
                            PS->>Outbox: Store authentication_session.created
                            PS->>Outbox: Store payment_attempt.requires_action
                            ...
                            alt PSP sends final webhook
                                PSP->>WH: payment final status webhook
                                WH->>WH: Validate signature<br/>Check webhook idempotency
                                WH->>PS: Apply final PSP result
                                PS->>Lock: Acquire lock(payment_id)
                                PS->>DB: Begin transaction
                                PS->>TS: Create final Transaction<br/>SUCCESS or FAILED
                                TS->>DB: Insert append-only Transaction
                                PS->>DB: Update Authentication Session final
                                PS->>AS: Mark Attempt final
                                AS->>DB: Update Attempt final
                                PS->>DB: Update Payment final
                                PS->>Outbox: Store transaction.created
                                PS->>Outbox: Store authentication_session.completed / failed
                                PS->>Outbox: Store payment_attempt.succeeded / failed
                                PS->>Outbox: Store payment.succeeded / failed
                                PS->>DB: Commit transaction
                                PS->>Lock: Release lock(payment_id)
                                WH-->>PSP: 200 ACK
                            else Webhook delayed or missing
                                Sync->>PSP: Retrieve payment/authentication status
                                PSP-->>Sync: Final or pending status
                                Sync->>PS: Apply PSP status if final
                            end

                        else Authentication failed synchronously
                            PSP-->>PA: Authentication failed
                            PA-->>PS: Normalized FAILED

                            PS->>DB: Begin transaction
                            PS->>TS: Create Transaction FAILED<br/>reason=AUTHENTICATION_FAILED
                            TS->>DB: Insert append-only Transaction
                            ...
                            Sync->>PSP: Retrieve final status
                            PSP-->>Sync: Final or pending status
                        end
